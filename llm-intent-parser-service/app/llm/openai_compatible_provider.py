...
class OpenAICompatibleProvider:
    def __init__(self, settings: Settings, retry_policy: LLMRetryPolicy | None = None) -> None:
        self._client = ChatOpenAI(
            model=settings.intent_parser_llm_model,
            base_url=settings.openai_compatible_base_url,
            api_key=settings.openai_compatible_api_key,
            temperature=settings.intent_parser_llm_temperature,
            max_tokens=settings.intent_parser_max_output_tokens,
            timeout=settings.intent_parser_llm_timeout_seconds,
            max_retries=0,
        )
